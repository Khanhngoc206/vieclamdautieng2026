...
// Fix cho Railway/proxy (bắt buộc khi deploy trên Railway)
...
app.post('/api/admin/jobs', requireAdmin, (req, res) => {
  ...
  const result = db.prepare(`INSERT INTO jobs (company,title,qty,category,salary,work_type,address,description,contact,phone,email,badge) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
